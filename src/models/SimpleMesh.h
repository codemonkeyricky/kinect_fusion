...
public:
	...
	static SimpleMesh sphere(Vector3f center, float scale = 1.f, Vector4uc color = { 0, 0, 255, 255 }) {
		...
		const std::vector<float> vertexComponents = { -0.525731, 0, 0.850651 ,0.525731, 0 ,0.850651, -0.525731, 0 ,-0.850651, 0.525731, 0 ,-0.850651, 0, 0.850651, 0.525731, 0, 0.850651, -0.525731, 0,
	...
	static SimpleMesh camera(const Matrix4f& cameraPose, float scale = 1.f, Vector4uc color = { 255, 0, 0, 255 }) {
		...
		// These are precomputed values for sphere aproximation.
		std::vector<float> vertexComponents = { 25, 25, 0, -50, 50, 100, 49.99986, 49.9922, 99.99993, -24.99998, 25.00426, 0.005185,
